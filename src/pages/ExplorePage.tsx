...
import { getScene, getTopic, vocabulary } from '../data';
...
function ExploreScene({ sceneId }: { sceneId: string }) {
  ...
  return <Layout><section className="page-heading explore-heading">
    ...
    <div className="explore-layout"><div><SceneArt scene={scene} discovered={explored} onTap={discover} highlight={hint} />
    ...
    {complete && <section className="completion-panel"><div><p className="eyebrow">Exploration complete</p><h2>You found them all!</h2><p>Put your new words into practice, or take another look.</p></div>
      <div className="button-row"><button className="button primary" onClick={start}>Start Challenge →</button>
        ...
        <Link className="button secondary" to={`/review/${scene.id}`}>Review Words</Link><Link className="text-button" to="/">Back to Topics</Link></div></section>}
