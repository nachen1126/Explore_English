...
export default function CategoryPage() {
  ...
  const label = count === kitchenScene.vocabularyIds.length ? '复习' : count > 0 ? '继续' : '开始探索';
  ...
  return <View className='page category-page'>
    ...
    <Text className='eyebrow'>选择场景</Text><Text className='title'>饮食篇</Text>
    <Text className='muted'>从厨房开始，在真实场景中学习单词。</Text>
    ...
    {view !== 'plans' && visible ? <View className='card scene-card' onClick={() => Taro.navigateTo({ url: '/pages/scene/index' })}>
      ...
      <Text className='scene-title'>{uiCopy.kitchenTitle}</Text>
      <Text className='scene-english-name'>{kitchenScene.title}</Text>
      <Text className='muted'>已发现 {count}/{kitchenScene.vocabularyIds.length} 个单词</Text>
    ...
    {view === 'plans' ? <View className='card coming-soon'><Text>更多场景正在准备中</Text><Text className='muted'>其他网页版场景会在逐一迁移并校准后发布，不展示假入口。</Text></View> : null}
  </View>;
}
