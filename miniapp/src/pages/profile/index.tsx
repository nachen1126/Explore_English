...
import { syncStateLabel } from '../../ui/copy';
import './index.scss';
...
export default function ProfilePage() {
  ...
  return <View className='page profile-page'>
    <Text className='eyebrow'>账户与同步</Text><Text className='title'>{user ? '微信账户' : '游客试用'}</Text>
    {user ? <View className='card'>
      {user.avatar ? <Image className='profile-avatar' src={user.avatar} mode='aspectFill' /> : null}
      <Text className='label'>用户 ID</Text><Text className='user-id'>{user.id}</Text>
      ...
      <Text className='muted'>同步状态：{syncStateLabel(syncState)}</Text>
