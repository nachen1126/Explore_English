...
export default function ScenePage() {
  ...
  function selectWord(vocabularyId: string) {
    ...
    void playPronunciation(vocabularyId, error => {
      if (pronunciationRequest.current === request) setAudioMessage(`播放中断，请检查设备音量。${error.message}`);
    })
  ...
  async function startOver() {
    const choice = await Taro.showModal({
      title: '重新探索这个场景？',
      content: '这会清除 Kitchen 已发现物品，但不会删除已经完成的挑战成绩。',
  ...
  return <View className='page scene-page'>
    ...
    {selectedId ? <WordPanel item={kitchenVocabularyById[selectedId]} />
      : <View className='card'><Text className='section-title'>A scene full of words.</Text><Text>请先在图片中发现一个物品。</Text></View>}
    <View className='scene-tools'>
      {!complete ? <Button className='button primary' onClick={nextWord}>Next word · 下一个单词</Button> : null}
      ...
      }}>{complete ? 'Review an object · 复习物品' : 'Show me a hint · 提示'}</Button>
      <Button className='button subtle' onClick={() => void startOver()}>Start over · 重新开始</Button>
    </View>
    {complete ? <View className='completion card'>
      <Text className='section-title'>You found them all! · 全部找到</Text>
      {incompleteAttempt ? <Button className='button primary' onClick={() => Taro.navigateTo({ url: `/pages/challenge/index?attemptId=${incompleteAttempt.attemptId}` })}>Resume Challenge · 继续挑战</Button> : null}
      <Button className={`button ${incompleteAttempt ? '' : 'primary'}`} onClick={startChallenge}>Start New Challenge · 新挑战</Button>
      <Button className='button' onClick={() => Taro.navigateTo({ url: '/pages/review/index' })}>Review Words · 复习单词</Button>
    </View> : null}
  </View>;
}
