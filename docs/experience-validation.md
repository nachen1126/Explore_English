...
This pass keeps the `explore-english-v2` storage key, schema version, legacy scene IDs and first-answer scoring rule. It adds two final illustrated scenes, recalibrates all published hotspots, separates full and weak-practice results, and changes the directory and responsive layouts without replacing the existing application or routes.
...
- Original Kitchen, Airport, Gym and Supermarket: all original words retained. Large irregular objects use polygons, repeated visible objects have multiple valid regions, and small targets render above broad surface regions.
...
The production build was measured at 100% zoom in 1366 × 768, 1440 × 900 and 1920 × 1080 viewports. The following states kept their main controls inside the viewport with no document overflow: Home, all six Explore scenes (empty, selected and complete), Find ready/wrong, Produce empty/wrong/correct/third-error hint/revealed, full results and weak-practice results. Image and overlay edge deviation was 0 px; aspect-ratio rounding error stayed below 0.00002.

The Home directory shows all three ready categories first, including thumbnail, published-scene count and aggregate progress. Five smaller Coming Soon categories remain visible below. Continue Learning chooses the most recently visited published scene. No category pagination or hidden overflow is used.
...
The Vitest suite contains 153 passing tests across seven files. New coverage includes desktop focus and a continuous keyboard sequence, touch-pointer focus suppression, hidden Chinese until three valid errors, multi-region hotspot discovery, recommendation priority, weak/full attempt separation, result reload consistency, home ordering and removal of user-visible development labels. Existing tests continue to cover legacy storage loading, first-answer scoring, Find auto-advance, duplicate click protection, speech failure classification and unmount cleanup.
